...
function printHelp() {
  console.log(`
Uso:
  node scripts/set-provider-items-accountid.js [opciones]
...
Ejemplos:
  node scripts/set-provider-items-accountid.js
  node scripts/set-provider-items-accountid.js --apply
  node scripts/set-provider-items-accountid.js --apply --env production
`);
}
  ...
  while (true) {
    ...
    for (const companyDoc of companiesSnap.docs) {
      ...
      for (const itemDoc of itemsSnap.docs) {
        scannedItems++;
        const currentAccountId = itemDoc.data()?.accountId;
        if (currentAccountId === ACCOUNT_ID) continue;

        changedItems++;
        if (!args.apply) {
          console.log(
            `[dry-run] ${itemDoc.ref.path}: ${currentAccountId || "(sin accountId)"} -> ${ACCOUNT_ID}`,
          );
          continue;
        }

        batch.update(itemDoc.ref, { accountId: ACCOUNT_ID });
        ops++;
