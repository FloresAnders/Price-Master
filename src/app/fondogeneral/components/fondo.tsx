      ...
      try {
        ...
        const loadRemoteEntries = async (
          ...
          try {
            ...
            return {
              result: buildEntriesFromStorage(remoteStorage, v2Movements),
              status: "success",
            };
          } catch (err) {
            console.error(
    ...
    async (
      ...
      try {
        ...
        if (operationType === "delete") {
          ...
          await MovimientosFondosService.upsertMovement(companyKey, {
            ...movement,
            accountId: accountKey,
    ...
    >
      ...
      {dailyClosingHistoryOpen && (
        <div
          ...
          onClick={() => setDailyClosingHistoryOpen(false)}
        >
          <div
            className="w-full max-w-3xl rounded border border-[var(--input-border)] bg-[#1f262a] p-6 shadow-lg text-white max-h-[80vh] overflow-auto"
            onClick={(e) => e.stopPropagation()}
            role="dialog"
            aria-modal="true"
              ...
              {closingsAreLoading ? (
                ...
                <div className="space-y-4">
                  {visibleDailyClosings.map((record) => {
                    ...
                    const crcLines = buildBreakdownLines(
                      "CRC",
                      record.breakdownCRC
                    );
                    const usdLines = buildBreakdownLines(
                      "USD",
                      record.breakdownUSD
                    );
                    const showCRC =
                          ...
                          <div className="rounded border border-[var(--input-border)]/60 bg-[var(--muted)]/10 p-3">
                            ...
                            {showCRC ? (
                              <div className="mt-2 space-y-1 text-sm text-[var(--foreground)]">
                                ...
                                <div
                                  className={getDailyClosingDiffClass(
                                    record.diffCRC
                                  )}
                                >
                                  Diferencia:{" "}
                        ...
                        {(() => {
                              ...
                              <div className="mt-3">
                                <div className="flex items-center justify-between p-3 rounded border-l-4 border-green-500 bg-green-900/5 text-sm">
                                  <div>
                                    ...
                                    <div className="text-xs text-[var(--muted-foreground)]">
                                      Los ajustes previos fueron eliminados y el
                                      saldo quedó normalizado.
                                    </div>
                                  </div>
                                  <button
                                    type="button"
                                  ...
                                  >
                                    ...
                                    <div className="text-xs text-[var(--input-border)]">
                                      ...
                                      record.adjustmentResolution
                                        ...
                                        <ul className="list-disc pl-5 text-[var(--muted-foreground)]">
                                          <li>
                                            Los ajustes asociados a este cierre
                                            fueron eliminados manualmente.
                                          </li>
                                          <li>
                          ...
                          if (relatedAdjustments.length > 0) {
                            const postAdjBalanceCRC =
                              record.adjustmentResolution
                                ?.postAdjustmentBalanceCRC;
                            const postAdjBalanceUSD =
                              record.adjustmentResolution
                                ?.postAdjustmentBalanceUSD;
