...
import type { RegistroTiemposRecord } from "../../shared/types/firestore";
import { iconBoxClass, sectionClass } from "./styles";
import type { RegistroTiemposSortOrder } from "./types";

type RegistroTiemposColumnKey =
  | "fecha"
  | "hora"
  | "saldoPaginaTiempos"
  | "saldoFondoTiempos"
...
type RegistroTiemposColumn = {
  key: RegistroTiemposColumnKey;
  label: string;
  defaultWidth: number;
  minWidth: number;
};

const COLUMN_WIDTHS_PREF_KEY = "registro-tiempos-records-column-widths";
const MAX_COLUMN_WIDTH = 800;
const KEYBOARD_RESIZE_STEP = 10;
const KEYBOARD_RESIZE_LARGE_STEP = 40;

const BASE_COLUMNS: RegistroTiemposColumn[] = [
  { key: "fecha", label: "Fecha", defaultWidth: 120, minWidth: 90 },
  { key: "hora", label: "Hora", defaultWidth: 100, minWidth: 80 },
  {
    key: "saldoPaginaTiempos",
    label: "Pagina Tiempos",
    defaultWidth: 160,
    minWidth: 120,
  },
  {
    key: "saldoFondoTiempos",
    label: "Fondo Tiempos",
    defaultWidth: 160,
    minWidth: 120,
  },
...
const ACTIONS_COLUMN: RegistroTiemposColumn = {
  key: "acciones",
  label: "Acciones",
  defaultWidth: 100,
  minWidth: 90,
...
function buildDefaultColumnWidths(): Record<RegistroTiemposColumnKey, number> {
  return [...BASE_COLUMNS, ACTIONS_COLUMN].reduce(
    (acc, column) => {
      acc[column.key] = column.defaultWidth;
      return acc;
    },
    {} as Record<RegistroTiemposColumnKey, number>,
  );
}

function clampColumnWidth(value: number, column: RegistroTiemposColumn): number {
  return Math.max(column.minWidth, Math.min(MAX_COLUMN_WIDTH, value));
}

type RegistroTiemposRecordsProps = {
  records: RegistroTiemposRecord[];
  recordsLoading: boolean;
  sortOrder: RegistroTiemposSortOrder;
  canDeleteRecords: boolean;
  onSortOrderChange: (value: RegistroTiemposSortOrder) => void;
  onRefresh: () => void;
  onDelete: (record: RegistroTiemposRecord) => void;
  formatCRC: (value: number) => string;
};

export function RegistroTiemposRecords({
  records,
  recordsLoading,
  sortOrder,
  canDeleteRecords,
  ...
  const [columnWidths, setColumnWidths] = useState<
    Record<RegistroTiemposColumnKey, number>
  ...
  const persistColumnWidths = useCallback(
    (widths: Record<RegistroTiemposColumnKey, number>) => {
  ...
  const startColumnResize = useCallback(
    (event: React.PointerEvent, key: RegistroTiemposColumnKey) => {
  ...
  const adjustColumnWidth = useCallback(
    (key: RegistroTiemposColumnKey, delta: number) => {
      const column = columnsByKey.get(key);
      if (!column) return;
  ...
  const handleColumnResizeKeyDown = useCallback(
    (event: React.KeyboardEvent, key: RegistroTiemposColumnKey) => {
              ...
              <tbody>
                {records.map((record) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-[var(--foreground)]">
                      {formatCRC(record.saldoPaginaTiempos)}
                    </td>
                    <td className="px-3 py-2 text-[var(--foreground)]">
                      {formatCRC(record.saldoFondoTiempos)}
                    </td>
                    <td className="px-3 py-2 text-[var(--foreground)]">
                      {formatCRC(
