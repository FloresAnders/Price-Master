...
export function RightPanel({
  ...
  const ntf = (
    ...
    onUpdate({
      ...
      extraAmount: e,
      currency: c,
      aperturaCaja: a ?? data.aperturaCaja,
      ventaActual: v ?? data.ventaActual,
    });
  ...
  const [showMobileTotal, setShowMobileTotal] = useState(true);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      setShowMobileTotal(!entry.isIntersecting);
    });
  ...
  const safeName =
    data.name.replace(/[^\w-]+/g, "_").replace(/^_+|_+$/g, "") ||
    "cash-counter";
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      const summary = buildCashCounterExportSummary(data);
      {
        ...
        host.innerHTML = buildCashCounterPreviewHtml(
          summary,
      ...
      const diffText =
        summary.difference.type === "sobrante"
      ...
      summary.denoms.forEach((denom) => {
        ...
        doc.setTextColor(
          denom.count > 0 ? 245 : 120,
          denom.count > 0 ? 250 : 130,
          denom.count > 0 ? 255 : 145,
        );
  ...
  return (
    <div className="w-full lg:w-[19.5rem] flex-shrink-0 order-3 lg:order-3">
      {showMobileTotal && (
        <div
          role="status"
          aria-label="Saldo total flotante"
          className="fixed inset-x-3 bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-40 flex items-center rounded-2xl border border-cyan-400/25 bg-[#0d1117]/95 px-4 py-3 shadow-[0_12px_40px_rgba(0,0,0,0.55)] backdrop-blur-xl lg:hidden"
        >
          ...
          <span className="absolute left-1/2 -translate-x-1/2 whitespace-nowrap text-xl font-semibold tracking-tight text-white">
            {fmtCurrency(t, cur)}
          </span>
        </div>
      )}
      <div className="lg:sticky lg:top-20 space-y-3">
        <div
          ref={summaryRef}
          className="rounded-2xl border border-white/10 bg-[#0d1117] p-5 text-center"
        >
          ...
          <p className="text-[10px] text-white/30 uppercase tracking-[0.18em] mb-1.5">
            Resumen
          </p>
          <p className="text-[2rem] leading-none font-semibold text-white tracking-tight">
            {fmtCurrency(t, cur)}
          </p>
          <div className="mt-4 pt-4 border-t border-white/10 space-y-3 text-xs">
            <div className="text-center">
              <span className="text-white/45 block mb-0.5">Billetes</span>
              <span className="text-white/90 font-medium">
                {fmtCurrency(t - extra, cur)}
              </span>
            </div>
            <div className="text-center">
              <span className="text-white/45 block mb-0.5">Total Billetes</span>
              <span className="text-white/90 font-medium">
                {Object.values(bills).reduce((a, b) => a + b, 0)}
              </span>
            </div>
            <div className="text-center">
              <span className="text-white/45 block mb-0.5">Extra</span>
              <span className="text-white/90 font-medium">
                {fmtCurrency(extra, cur)}
              </span>
            </div>
            <div className="text-center">
              <span className="text-white/45 block mb-0.5">Activas</span>
              <span className="text-white/90 font-medium">
                {Object.values(bills).filter((c) => c > 0).length}/
                {denoms.length}
              </span>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-[#0d1117] p-4">
          <p className="text-[10px] text-white/30 uppercase tracking-[0.16em] mb-3">
            Acciones rápidas
          </p>
          <div className="space-y-2">
            ...
            >
              ...
              {showExtra ? "Cerrar Extra" : "Monto Adicional"}
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            ...
            >
              ...
              {showBD ? "Ocultar Desglose" : "Ver Desglose"}
            </motion.button>
          </div>
        </div>
          ...
          <div className="flex items-center justify-center gap-2">
            {[
                    ...
                    new FileReader().onload = (e) => {
                      try {
                        ...
                        if (
                          ...
                          typeof p.extraAmount === "number" &&
                          (p.currency === "CRC" || p.currency === "USD") &&
                          typeof p.bills === "object"
                        ) {
                          ntf(
                            p.bills,
                            p.extraAmount,
              ...
              <button
                key={b.label}
                onClick={b.action}
                className={`w-8 h-8 rounded-lg ${b.bg} border ${b.border} flex items-center justify-center ${b.color} transition-all`}
                aria-label={b.label}
              >
        ...
        <AnimatePresence>
          {showExtra && (
            ...
            >
              <p className="text-[10px] text-white/35 uppercase tracking-[0.16em]">
                Extra
              </p>
              <div className="bg-[#050816] border border-white/10 rounded-xl p-2 flex items-center">
                <input
                  type="text"
                  inputMode="numeric"
                  value={extra === 0 ? "" : fmtCurrency(extra, cur)}
                  onChange={(e) => {
                    let r = e.target.value;
                    if (cur === "CRC") {
                      r = r.replace(/\D/g, "");
                      ntf(bills, r === "" ? 0 : parseInt(r, 10), cur);
                    } else {
                      r = r.replace(/[^0-9.]/g, "");
                      const p = parseFloat(r);
                      ntf(bills, isNaN(p) ? 0 : p, cur);
                    }
                  }}
                ...
                />
                <button
                  onClick={() => ntf(bills, 0, cur)}
                  className="ml-2 text-white/20 hover:text-red-400/60 transition-colors"
                >
        ...
        <AnimatePresence>
          {confirmOpen && (
            ...
            >
              <p className="text-center text-white/80 mb-4">
                ¿Estás seguro de eliminar este contador?
              </p>
              <div className="flex justify-end gap-2">
                ...
                <motion.button
                  ...
                  className="flex items-center gap-1 px-4 py-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-400"
                >
          ...
          {clearConfirmOpen && (
            ...
            >
              <p className="text-center text-white/80 mb-4">
                ¿Estás seguro de limpiar este contador?
              </p>
              <div className="flex justify-end gap-2">
                <motion.button
                  whileHover={{ scale: 1.02 }}
                ...
                <motion.button
                  ...
                  className="flex items-center gap-1 px-4 py-2 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-400"
                >
