...
export default function ImportEmpresas() {
    ...
    const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            } else if (parsed && typeof parsed === 'object' && Array.isArray(parsed.empresas)) {
                dataArray = parsed.empresas;
            } else {
            ...
            if (!confirm(`¿Importar ${dataArray.length} empresas?`)) {
                setImporting(false);
                return;
            }
            ...
            for (const item of dataArray) {
                try {
                    ...
                    if (docId) {
                        await setDoc(doc(db, 'empresas', docId), docData);
                        updated++;
                    } else {
                        await addDoc(collection(db, 'empresas'), docData);
                        imported++;
                    }
