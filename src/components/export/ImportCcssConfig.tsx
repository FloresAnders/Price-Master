...
export default function ImportCcssConfig() {
    ...
    const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            } else if (parsed && typeof parsed === 'object' && parsed.ownerId) {
                // Formato antiguo de config única
                dataArray = [parsed];
            } else {
            ...
            if (!confirm(`¿Importar ${dataArray.length} configuraciones CCSS?`)) {
                setImporting(false);
                return;
            }
            ...
            for (const item of dataArray) {
                try {
                    ...
                    if (docId) {
                        await setDoc(doc(db, 'ccss-config', docId), docData);
                        updated++;
                    } else {
                        await addDoc(collection(db, 'ccss-config'), docData);
                        imported++;
                    }
