...
export default function ImportSolicitudes() {
    ...
    const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (!confirm(`¿Importar ${dataArray.length} solicitudes?`)) {
                setImporting(false);
                return;
            }
            ...
            for (const item of dataArray) {
                try {
                    ...
                    if (docId) {
                        await setDoc(doc(db, 'solicitudes', docId), docData);
                    } else {
                        await addDoc(collection(db, 'solicitudes'), docData);
                    }
                    imported++;
            ...
            if (errors > 0) {
                ...
                alert(`✅ Importación exitosa! ${imported} solicitudes importadas.`);
            }
        } catch (err) {
