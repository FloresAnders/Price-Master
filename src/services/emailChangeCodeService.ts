...
export class EmailChangeCodeService {
  ...
  static async createCode(userId: string): Promise<{ code: string; expiresAt: number }> {
  ...
  static async consumeCode(params: { userId: string; code: string }): Promise<void> {
    const codeHash = this.hashCode(params.code);
  ...
  private static async logSecurityEvent(type: string, payload: Record<string, unknown>): Promise<void> {
    try {
